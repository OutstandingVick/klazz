  ...
  return <main className="shell">
    ...
    <section className="workspace">
      ...
      {result && <section className={`answerCard ${result.state !== "answer" ? "abstainCard" : ""}`} aria-live="polite"><div className="answerHead"><span className="stateBadge">{result.state === "conflict" ? "Conflicting memories" : result.temporalStatus === "current" ? "Current state" : result.temporalStatus === "historical" ? "Historical state" : "No supported memory"}</span><span className="verified">Verified from HydraDB</span></div><h2>{result.answer}</h2><p>{result.temporalStatus === "historical" ? `As of June 30, 2026, the ${result.answer.toLowerCase().includes("employee") ? "recorded headcount" : "planned launch date"} was ${result.answer}. It was active at that time${result.evidence[0]?.status === "superseded" ? " and has since been superseded" : ""}.` : result.explanation}</p>
        ...
        {evidenceOpen && result.evidence.length > 0 && <div className="evidenceGrid">{result.evidence.map(item => <article key={item.sessionId}><div><span className={`evidenceState ${item.status}`}>{result.temporalStatus === "historical" && item.status === "superseded" ? "Active at that time — now superseded" : item.status}</span><time>{item.eventTime.slice(0,10)}</time></div><strong>{item.value}</strong><code>{item.sessionId}</code></article>)}</div>}
