import { classifyQuestion, cypherFor, shapeResult } from "../../../lib/klazz";
...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(`${baseUrl}/v1/graphs/default/query`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "X-Graph-Namespace": "default", "Content-Type": "application/json" }, body: JSON.stringify({ cell_id: "cell-0", query: cypherFor(kind), consistency: "strong" }), signal: controller.signal });
    const body = await response.json() as { error?: { message?: string } };
    if (!response.ok) throw new Error(body.error?.message ?? `HydraDB returned HTTP ${response.status}`);
    return Response.json(shapeResult(kind, body));
  } catch (error) {
